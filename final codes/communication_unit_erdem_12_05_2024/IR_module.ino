...
void shift_reg_insert_zero_fast() {
  PORTD = PORTD & B11111011;  //define SHIFT_REG_INPUT 5 (portd-5)->LOW
  //digitalWrite(SHIFT_REG_INPUT, LOW);
  delayMicroseconds(1);

  PORTD = PORTD & B11111101;  //#define SHIFT_REG_CLK_PIN 6 (portd-6)->LOW
  //digitalWrite(SHIFT_REG_CLK_PIN, LOW);
  delayMicroseconds(1);

  PORTD = PORTD | B00000010;  //#define SHIFT_REG_CLK_PIN 6 (portd-6)->HIGH
  //digitalWrite(SHIFT_REG_CLK_PIN, HIGH);
  delayMicroseconds(1);
}

void shift_reg_insert_one_fast() {
  PORTD = PORTD | B00000100;  //define SHIFT_REG_INPUT 5 (portd-5)->HIGH
  //digitalWrite(SHIFT_REG_INPUT, LOW);
  delayMicroseconds(1);
