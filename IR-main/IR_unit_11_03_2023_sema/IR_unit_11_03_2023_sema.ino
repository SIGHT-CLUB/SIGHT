...
uint8_t mode_of_operation = 0; //0:listen, 1:ping, 2: ack, 3: send data
...
void transmit_data(){
  ...
  uint8_t first_ID = 2; //4 bits

  uint8_t first_first_byte = first_x*16 + first_y; //x and y position of the robot
  uint8_t first_second_byte = first_intention*16 + first_ID; //intention and ID of the robot

  //TRANSMIT ---------------
  set_buffer(0, first_first_byte);
  set_buffer(1, first_second_byte);

  uint16_t first_CRC_16 = generate_CRC_16_bit();
  uint8_t first_CRC_SIG = first_CRC_16 >> 8;
  uint8_t first_CRC_LST = first_CRC_16 % 256;
  set_buffer(2, first_CRC_SIG);
  set_buffer(3, first_CRC_LST);
  ...
  /// SECOND PACKAGE ///
  Serial.println("\n Transmitting Second Package...");
  //INFO ---------------
  uint8_t second_x = 8; // 4 bits
  uint8_t second_y = 9; // 4 bits
  
  uint8_t second_intention = 3; //4 bits
  uint8_t second_ID = 2; //4 bits

  uint8_t second_first_byte = second_x*16 + second_y; //x and y position of the robot
  uint8_t second_second_byte = second_intention*16 + second_ID; //intention and ID of the robot

  //TRANSMIT ---------------
  set_buffer(0, second_first_byte);
  set_buffer(1, second_second_byte);

  uint16_t second_CRC_16 = generate_CRC_16_bit();
  uint8_t second_CRC_SIG = second_CRC_16 >> 8;
  uint8_t second_CRC_LST = second_CRC_16 % 256;
  set_buffer(2, second_CRC_SIG);
  set_buffer(3, second_CRC_LST);
