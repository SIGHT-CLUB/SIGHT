...
void ping_test() {
  ...
  while(millis() - start_time < delay_after_ping){
    ...
    } else mode_of_operation = 0; //if no ack is received, change the mode to listen for ack
  }
...
void listen_for_ping_test(){
  ...
  // ping is 4 bytes to change the number of bytes accordingly
  set_number_of_package_bytes(4);

  // keep the info of whether the ack is sent
  uint8_t ack_sent = 0;

  Serial.println("\n Currently Listening for Ping...");

  // set the listen time for ping
  unsigned long listen_duration = random(1000,2000);
  // Serial.println("\nThe random number is: ");

  // check the time to listen for ping
  unsigned long start_time = millis() ;
  ...
  while(millis() - start_time < listen_duration){
    ...
    if (listening_result == 1){
      ...
      if (incoming_intention == 1){
        ...
        // set the ack_sent and mode of operation accordingly
        ack_sent = 1;
  ...
  if(ack_sent == 0) mode_of_operation = 1; //if no ping is received, change to ping mode
}
