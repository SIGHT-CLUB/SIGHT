...
uint8_t listen_IR() {
  ...
  if (is_received == 1) {
    ...
    if (IR_module_buffer[NUMBER_OF_PACKAGE_BYTES - 1] == CRC_8) {
...
//MAGICAL CRC_8 MODBUS code
uint8_t generate_CRC_8_bit() {
  uint8_t remainder = CRC_8_bit_for_1BYTE(IR_module_buffer[0], 0xFF);
  for (uint8_t i = 1; i < NUMBER_OF_PACKAGE_BYTES - 1; i++) {
    remainder = CRC_8_bit_for_1BYTE(IR_module_buffer[i], remainder);
  }
  return remainder;
}

uint8_t CRC_8_bit_for_1BYTE(uint8_t data, uint8_t last_data) {
  uint8_t key = 0xF4;  // Polynomial: x^8 + x^4 + x^3 + x^2 + 1
  data = data ^ last_data;  // XOR
  for (int i = 0; i < 8; i++) {
    bool should_XOR = (data & 0x80) != 0;  // Check the MSB
    data = data << 1;
    if (should_XOR) {
      data = data ^ key;
    }
