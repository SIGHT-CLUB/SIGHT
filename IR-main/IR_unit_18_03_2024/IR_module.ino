...
void transmit_buffer() {
  ...
  unsigned long time_node = micros();
  ...
  for (uint16_t byte_index = 0; byte_index < NUMBER_OF_PACKAGE_BYTES; byte_index++) {
    for (uint8_t package_bit = 0; package_bit < 8; package_bit++) {
      ...
      time_node = time_node + TRIGGER_DURATION_US;
      while(micros()<time_node);
...
void transmit_zero() {
  TRANSMISSION_START_TIME = micros()-50;
  while (micros() - TRANSMISSION_START_TIME < (TRIGGER_DURATION_US)) {
    // one iteration takes 32 us
...
void transmit_one() {
  ...
  // one iteration takes 12 us
  digitalWrite(IR_LED, LOW);
  delayMicroseconds(TRIGGER_DURATION_US-50);
}
